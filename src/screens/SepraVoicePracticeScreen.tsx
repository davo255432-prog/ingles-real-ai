...
interface SepraVoicePracticeScreenProps {
  onBack: () => void;
  onCorrection: () => void;
}

export const SepraVoicePracticeScreen: React.FC<SepraVoicePracticeScreenProps> = ({
  onBack,
  onCorrection,
}) => {
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <div className="flex-1 px-5 pt-12 pb-4">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-blue-600">Vamos a mejorar tu pronunciación</h1>
        </div>

        <SepraStepIndicator currentStep={2} />

        {/* Phrase */}
        <div className="bg-blue-500 rounded-2xl p-5 mb-6 text-center">
          <p className="text-blue-100 text-xs font-semibold uppercase tracking-wide mb-2">Frase para practicar</p>
          <p className="text-white text-2xl font-extrabold">"I don't understand."</p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            <div className={['w-6 h-6 rounded-full border-2 border-current flex items-center justify-center', voiceState === 'listening' ? 'bg-white' : ''].join(' ')}>
              {voiceState === 'listening'
                ? <span className="w-3 h-3 bg-red-500 rounded-full" />
                : <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z" /></svg>
              }
            </div>
            {voiceState === 'listening' ? 'Grabando...' : 'Presiona para hablar'}
          </button>

          <Button onClick={handleRepeat} color="gray" variant="outline" size="md" fullWidth
            icon={
